# Exercise 09 — Build remote agents with A2A

Multiple specialized agents (title, outline, routing) exposed over the **Agent2Agent (A2A) protocol** and coordinated by a routing agent.

> **Official exercise:** [09-multi-remote-agents-with-a2a.html](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/Exercises/09-multi-remote-agents-with-a2a.html)
...
This exercise starts several local agent servers (see `src/*_agent/`). `run_all.py` launches them together; ports are configured in `.env`.
